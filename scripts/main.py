...
def on_ui_tabs():
  png_input_area = gr.Image(label="Select")
  with gr.Blocks(analytics_enabled=False) as openpose_editor:
          ...
          with gr.Column(scale=3):
            ...
            for t in types:
              with gr.Tab(t.capitalize()):
                dataset = gr.Examples(examples=os.path.join(maps_path, t),
                                      inputs=[png_input_area],
                                      examples_per_page=24, label="Hands Maps", elem_id="examples")

          with gr.Column(scale=1):
            png_input_area.render()
            add_select = gr.Button(value="Add selected image", variant="primary")
    ...
    # add_select.click(pp, [png_input_area], None, _js="")
    add_select.click(None, [png_input_area], None, _js="(path) => {keypoint_addImg(path)}")
